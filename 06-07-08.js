...
function crearClaseMascota() {
  class Mascota {
    ...
    getActividades() {
      ...
      // Tu código aca:
      let acti = this.actividades.map(function(element){
        return element.actividad
      })
      return acti;      
    }
    ...
    getFrecuencia(actividad) {
      ...
      // Tu código aca:
      let claves = Object.keys(this.actividades); 
      for (let i = 0; i < claves.length; i++){
        if(this.actividades[claves[i]].actividad == ('salir a caminar') ){
            return this.actividades[claves[i]].frecuencia;
        }
      }
